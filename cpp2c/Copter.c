...
//private:
    ...
    struct MultiCopter aparm;

    // cliSerial isn't strictly necessary - it is an alias for hal.console. It may
    // be deprecated in favor of hal.console in later releases.
    //AP_HAL::BetterStream* cliSerial;
    struct BetterStream cliSerial;

    // Global parameters are all contained within the 'g' class.
    //Parameters g;
    struct Parameters g;
    //ParametersG2 g2;
    struct ParametersG2 g2;

    // main loop scheduler
    //AP_Scheduler scheduler;
    struct AP_Scheduler scheduler;
   // // AP_Notify instance
    //AP_Notify notify;
    struct AP_Notify notify;
    ...
    // size of bool can be 1byte or 4bytes; assume it is 4bytes now
    int in_log_download;
    ...
    struct {
        ...
        int alt_healthy:1; // true if we can trust the altitude from the rangefinder
        int16_t alt_cm;     // tilt compensated altitude (in cm) from rangefinder
        uint32_t last_healthy_ms;
        //LowPassFilterFloat alt_cm_filt; // altitude filter
        struct LowPassFilterFloat alt_cm_filt; // altitude filter
        int8_t glitch_count;
